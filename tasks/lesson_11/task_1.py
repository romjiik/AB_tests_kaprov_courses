...
class SplittingService:
    ...
    def add_experiment(self, experiment):
        ...
        if len(experiment.conflicts) == 0:
            free_buckets = [idx for idx in range(self.buckets_count)]
        else:
            for conflict in experiment.conflicts:
                for idx, bucket in enumerate(self.buckets):
                        if conflict not in bucket:
                            free_buckets.append(idx)

        if experiment.buckets_count <= len(free_buckets):
            for bucket in free_buckets[:experiment.buckets_count]:
                self.buckets[bucket].append(experiment.id)
