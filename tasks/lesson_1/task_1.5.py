...
class MetricsService:
    ...
    def _calculate_revenue_web(self, begin_date, end_date, user_ids):
        ...
        :return (pd.DataFrame): датафрейм с двумя столбцами ['user_id', 'metric']
        """
        df_sales = self._get_data_subset('sales', begin_date, end_date, user_ids, ['user_id', 'price'])
        df_web_logs = self._get_data_subset('web-logs', begin_date, end_date, user_ids, ['user_id'])
        df = df_web_logs.drop_duplicates().merge(df_sales, how='left', on='user_id').fillna(0)
        df = df.groupby('user_id', as_index=False)['price'].sum()
        df.columns = ['user_id', 'metric']
        return df
    ...
    def _calculate_revenue_all(self, begin_date, end_date, user_ids):
        ...
        """
        df_sales = self._get_data_subset('sales', begin_date, end_date, user_ids, ['user_id', 'price'])
        df_web_logs = self._get_data_subset('web-logs', None, end_date, user_ids, ['user_id'])
        df = df_web_logs.drop_duplicates().merge(df_sales, how='left', on='user_id').fillna(0)
        df = df.groupby('user_id', as_index=False)['price'].sum()
        df.columns = ['user_id', 'metric']
        return df
