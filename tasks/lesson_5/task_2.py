...
class ExperimentsService:
    ...
    def _generate_bootstrap_metrics(self, data_one, data_two, design):
        ...
        :return bootstrap_metrics, pe_metric:
            bootstrap_metrics (np.array) - значения статистики теста посчитанное по бутстрепным подвыборкам
    ...
    def _run_bootstrap(self, bootstrap_metrics, pe_metric, design):
        ...
        if design.bootstrap_ci_type == 'normal':
            ci = [pe_metric - stats.norm.ppf(1 - design.alpha / 2) * np.std(bootstrap_metrics), 
                  pe_metric + stats.norm.ppf(1 - design.alpha / 2) * np.std(bootstrap_metrics)] 
            pvalue = 0 if (ci[0] > 0) or (ci[1] < 0) else 1
            return ci, pvalue
        if design.bootstrap_ci_type == 'percentile':
            ci = [np.quantile(bootstrap_metrics, design.alpha / 2), 
                  np.quantile(bootstrap_metrics, 1 - design.alpha / 2)] 
            pvalue = 0 if (ci[0] > 0) or (ci[1] < 0) else 1
            return ci, pvalue
        if design.bootstrap_ci_type == 'pivotal':
            ci = [2 * pe_metric - np.quantile(bootstrap_metrics, 1 - design.alpha / 2), 
                  2 * pe_metric - np.quantile(bootstrap_metrics, design.alpha / 2)] 
            pvalue = 0 if (ci[0] > 0) or (ci[1] < 0) else 1
            return ci, pvalue
