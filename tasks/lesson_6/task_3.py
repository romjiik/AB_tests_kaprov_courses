...
class MetricsService:
    ...
    def process_outliers(self, metrics, design):
        ...
        """
        new_metrics = metrics.copy(deep=True)
        if design.metric_outlier_process_type == 'drop':
            new_metrics = new_metrics.loc[(new_metrics['metric'] >= design.metric_outlier_lower_bound) &
                                          (new_metrics['metric'] <= design.metric_outlier_upper_bound), :]
            return new_metrics
        else:
            new_metrics.loc[new_metrics['metric'] < design.metric_outlier_lower_bound, 'metric'] = design.metric_outlier_lower_bound
            new_metrics.loc[new_metrics['metric'] > design.metric_outlier_upper_bound, 'metric'] = design.metric_outlier_upper_bound
            return new_metrics
