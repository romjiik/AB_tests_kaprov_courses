    ...
    def calculate_metric(self, metric_name, begin_date, end_date, cuped, user_ids=None):
        ...
        if metric_name == 'revenue (web)':
            if cuped == 'off':
                ...
                df_previous = self._calculate_revenue_web(begin_date - timedelta(days=7), end_date - timedelta(days=7), user_ids)
                df = df.merge(df_previous, how='left', on='user_id', suffixes=('', '_before')).fillna(0)
                theta = np.cov(df['metric'], df['metric_before'])[0, 1] / np.var(df['metric_before'])
                df['metric'] = df['metric'] - theta * (df['metric_before'] - np.mean(df['metric_before']))
                return df[['user_id', 'metric']]
            else:
